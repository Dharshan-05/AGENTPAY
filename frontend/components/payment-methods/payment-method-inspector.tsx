...
import {
  CreditCard, ShieldCheck, Activity, Lock, ArrowRight, Bot, AlertTriangle, Check
} from 'lucide-react';
import { PaymentInstrumentRecord } from './payment-method-types';

interface PaymentMethodInspectorProps {
  item: PaymentInstrumentRecord | null;
  onClose: () => void;
}

export function PaymentMethodInspector({ item, onClose }: PaymentMethodInspectorProps) {
      ...
      footer={
        <div className="space-y-2 font-mono">
          <div className="flex gap-2">
            <AGButton variant="ghost" size="sm" onClick={handleCopyPm} className="flex-1">
              {copiedPm ? <Check className="w-3 h-3 text-emerald-400 mr-1" /> : null}
              {copiedPm ? 'COPIED PM ID' : 'COPY PM ID'}
            </AGButton>
            <AGButton variant="ghost" size="sm" onClick={handleCopyToken} className="flex-1">
              {copiedToken ? <Check className="w-3 h-3 text-emerald-400 mr-1" /> : null}
              {copiedToken ? 'COPIED TOKEN' : 'COPY TOKEN ID'}
            </AGButton>
          </div>
