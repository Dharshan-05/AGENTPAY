...
import { Search, Bell, AlertOctagon, User, ShieldCheck, Check } from 'lucide-react';
...
export function TopNav({ searchQuery, onSearchChange, onEmergencyFreeze }: TopNavProps) {
  ...
  const ALERTS = [
    { id: 1, title: 'Anomalous Wire Blocked', time: '2m ago', severity: 'HIGH' },
    { id: 2, title: 'Ad Campaign Spend at 96%', time: '14m ago', severity: 'MEDIUM' },
    { id: 3, title: 'AGENTGUARD Policy Updated', time: '1h ago', severity: 'INFO' },
  ];
  ...
  return (
    <header className="h-16 bg-slate-950/90 backdrop-blur-xl border-b border-white/[0.08] px-6 flex items-center justify-between gap-4 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3 flex-1 max-w-lg">
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder="Search Agent ID, Intent, Policy, Hash, or Vendor..."
      ...
      <div className="flex items-center gap-4">
          ...
          {showAlerts && (
            <div className="absolute right-0 mt-2 w-80 bg-slate-950 border border-white/10 rounded-2xl shadow-2xl p-4 z-50 font-mono">
              <div className="flex items-center justify-between pb-3 border-b border-white/[0.08] mb-3 text-xs">
                <span className="font-bold text-slate-200">ACTIVE SECURITY ALERTS</span>
        ...
        <Link href="/login" className="flex items-center gap-3 pl-2 border-l border-white/[0.08] hover:opacity-80 transition-opacity" title="Sign In / Authenticate">
