...
import { RoutingDecisionRecord } from './source-types';

interface SourceRoutingProps {
  decisions: RoutingDecisionRecord[];
}

export function SourceRouting({ decisions }: SourceRoutingProps) {
  return (
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-white/[0.08] text-[10px] text-slate-500 uppercase tracking-wider">
            <th className="px-4 py-3 font-semibold">METHOD ID &amp; NAME</th>
            <th className="px-4 py-3 font-semibold">ORIGIN AGENT</th>
            <th className="px-4 py-3 font-semibold">GEO / CURRENCY</th>
            <th className="px-4 py-3 font-semibold">PRIMARY ROUTE</th>
            <th className="px-4 py-3 font-semibold">FALLBACK ROUTE</th>
          ...
          {decisions.map((dec) => (
            <tr key={dec.id} className="hover:bg-slate-900/40 transition-colors">
              ...
              <td className="px-4 py-3.5 font-bold">
                <span className={
                  dec.status === 'OPTIMAL_ROUTE' ? 'text-emerald-400' :
                  dec.status === 'FALLBACK_TRIGGERED' ? 'text-amber-400' :
                  'text-red-400'
                }>
                  {dec.status}
                </span>
