...
export function SourceHeader({ onSave, onReset }: SourceHeaderProps) {
  return (
    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4 font-sans text-slate-800">
      ...
      <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-slate-600 text-[11px]">supabase/supabase · keycloak/keycloak</span>
        </div>
        <span className="text-[10px] text-slate-500 font-medium">Route: /research/settings-source</span>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            ...
            Enterprise Security & Organization Settings Source UI
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Excavated information architecture reproducing account management, MFA, RBAC permission matrices, active session revocation, and security posture.
          </p>
        </div>
