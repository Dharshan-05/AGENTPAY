"""Unit tests for memory poisoning detection and quarantine in SecureMemoryRetriever."""
...
@pytest.mark.asyncio
async def test_01_poisoned_memory_quarantined():
    ...
    mock_recall_resp = AgentMemoryRecallResponse(
        query="spending",
        total_recalled=1,
        results=[AgentMemoryRecallItem(memory=poisoned_mem, relevance_score=0.95)],
    )
    ...
    assert len(res.memories) == 0  # Quarantined memory excluded from active recalled list
