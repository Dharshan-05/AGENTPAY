"""Unit & Mandatory Security Tests for BLOCK Decision Engine (Phase 280)."""
...
def test_01_policy_deny_triggers_block() -> None:
    """1. Mandatory Security Test: Policy DENY triggers BLOCK."""
    ctx = _make_context()
    ...
    is_block, reasons = engine.evaluate_block(ctx, calc_res, thresh_res, sec_res)

    assert is_block is True
    assert "POLICY_DENY_BLOCK" in reasons


def test_02_critical_security_rule_triggers_block() -> None:
    """2. Mandatory Security Test: Critical hard security rule triggers BLOCK."""
    ctx = _make_context()
    engine = BlockDecisionEngine()
    ...
    calc_res = RiskScoreCalculationResult(
        ...
        composite_risk_score_decimal=Decimal("5.0"),
        score_unit=RiskScoreUnit.RISK_SCORE,
        included_signal_types=["FRAUDGUARD"],
        excluded_signal_types=[],
        available_signal_types=["FRAUDGUARD"],
    ...
    thresh_res = RiskThresholdEvaluationResult(
        ...
        prediction_timestamp=ctx.prediction_timestamp,
        risk_score=5.0,
        risk_score_decimal=Decimal("5.0"),
        classification="LOW",
        matched_threshold_band=RiskThresholdBand.LOW_RISK_BAND,
        configuration_version="1.0.0",
        configuration_hash="t" * 64,
    ...
    assert is_block is True
    assert "CRITICAL_SECURITY_RULE_BLOCK_HSR-005" in reasons


def test_03_high_risk_score_triggers_block() -> None:
    """3. Test HIGH_RISK_BAND triggers BLOCK."""
    ctx = _make_context()
    engine = BlockDecisionEngine()
    ...
    assert is_block is True
    assert "HIGH_RISK_SCORE_BLOCK" in reasons
